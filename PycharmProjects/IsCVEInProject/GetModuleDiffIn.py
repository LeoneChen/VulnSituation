...
def is_only_non_add_diff_in_lines(diff, dst_line_list):
    return True if Repository.is_lines_in_lines(diff[1], dst_line_list) \
                   and not Repository.is_lines_in_lines(diff[2], dst_line_list) else False
...
def get_module_scope_tuple(module_info_list, dst_file_path, line_offset=0):
    ...
    for line_index in range(len(dst_file_line_list[line_offset:])):
        if match_lines(module_info_list, dst_file_line_list[line_offset + line_index:]):
...
#     write_all_diff_module_and_scope_tuple(r'C:\Users\79196\Downloads\NVD\Linux Kernel Patch')
